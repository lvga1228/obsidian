...
- **HTTP 400 on capture results** — Some providers (e.g. Xiaomi MiMo) don't
  support multimodal tool results (image_url in tool messages). Hermes
  auto-recovers by stripping images and retrying in text-only mode; SOM
  element indices survive as the text summary. If the error isn't detected
  automatically, see the hermes-agent skill's "Model/provider issues"
  troubleshooting section for how to add the provider's error phrase.
