...
13. **tmux may not be pre-installed on macOS** — `brew install tmux` can hang (120s+ timeout). Fallback: `conda install -y -c conda-forge tmux` (fast, installs to `$CONDA_PREFIX/bin/tmux`, v3.6a confirmed working). Check with `which tmux && tmux -V`.
14. **Non-Anthropic backends cause silent skill failures** — Claude Code can be configured with non-Anthropic models (e.g., `deepseek-v4-pro`). Skills in `~/.claude/skills/` will load but may produce zero output for 60+ seconds when the model can't execute the workflow. **Symptom**: process runs with no stdout for >60s. **Fix**: check `claude --model` or `claude auth status`; Anthropic-keyed models are required for complex skills like `ultimate-ppt-master`. Fallback: generate the deliverable directly (e.g., python-pptx for presentations).
15. **Multi-line prompts with `&` fail in Hermes terminal** — the terminal tool interprets `&` as backgrounding. **Workaround**: write the prompt to a file first (`write_file`), then use `cat prompt.txt | claude -p "$(cat prompt.txt)" ...` or send via tmux `send-keys` for interactive mode.
