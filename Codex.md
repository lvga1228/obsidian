...
- **China network**: Codex's Rust HTTP client does NOT respect `https_proxy`. Must `unset https_proxy http_proxy` before running. **OpenAI endpoints (`api.openai.com`, `chatgpt.com`) may be blocked by GFW** — always verify connectivity first: `curl -s -o /dev/null -w "%{http_code}" --connect-timeout 5 https://api.openai.com`. If unreachable, a system-level proxy (Surge/ClashX TUN mode) is required — env-var proxies won't work with Codex's Rust client. See `references/china-network-setup.md` for full diagnostic workflow.
...
```
OpenAI endpoints (`api.openai.com`, `chatgpt.com`) are sometimes blocked by GFW. Verify before running Codex: `curl -s -o /dev/null -w "%{http_code}" --connect-timeout 5 https://api.openai.com`. If 000 (timeout), Codex won't work without a system-level proxy.
```bash
...
Installed via the Codex primary runtime marketplace. Enable in `~/.codex/config.toml`:
...
| Plugin | What it does |
|--------|-------------|
| `hyperframes` | HTML-as-video compositions: CSS/GSAP animations, captions, audio-reactive visuals, transitions. Requires `hyperframes` CLI (npm) + Node.js ≥22 + FFmpeg. Plugin skills cached at `~/.codex/plugins/cache/openai-curated/hyperframes/` |
| `documents` | Document editing |
| `spreadsheets` | Spreadsheet manipulation |
| `presentations` | Presentation authoring |
| `gmail` | Gmail integration |

HyperFrames CLI install (China — use npmmirror if npm direct is slow):
```bash
sudo npm install -g hyperframes --registry=https://registry.npmmirror.com
```
